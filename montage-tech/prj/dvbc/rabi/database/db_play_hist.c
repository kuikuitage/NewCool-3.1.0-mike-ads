...
BOOL db_plht_get_item_by_idx(u16 index, play_hist_info *p_info)
{
	...
	{
	        memcpy(p_info->vdo_id.qpId, g_play_hist_item.vdo_id.qpId, (strlen(g_play_hist_item.vdo_id.qpId) + 1));
	        memcpy(p_info->vdo_id.tvQid, g_play_hist_item.vdo_id.tvQid, (strlen(g_play_hist_item.vdo_id.tvQid) + 1));
	        memcpy(p_info->vdo_id.sourceCode, g_play_hist_item.vdo_id.sourceCode, (strlen(g_play_hist_item.vdo_id.sourceCode) + 1));
	}
	else
	{
		memcpy(p_info->vdo_id.info_url, g_play_hist_item.vdo_id.info_url, (strlen(g_play_hist_item.vdo_id.info_url) + 1));
	}
        p_info->vdo_id.type = g_play_hist_item.vdo_id.type;
        p_info->res_id = g_play_hist_item.res_id;
        p_info->b_single_page = g_play_hist_item.vdo_id.type;
...
void db_plht_add_item(play_hist_info *p_info)
{
	...
	if (IPTV_ID_IQY ==  ui_iptv_dp_get_iptvId())
	{
	        memcpy(g_play_hist_item.vdo_id.qpId, p_info->vdo_id.qpId,(strlen(p_info->vdo_id.qpId) + 1));
	        memcpy(g_play_hist_item.vdo_id.tvQid, p_info->vdo_id.tvQid,(strlen(p_info->vdo_id.tvQid) + 1));
	        memcpy(g_play_hist_item.vdo_id.sourceCode, p_info->vdo_id.sourceCode,(strlen(p_info->vdo_id.sourceCode) + 1));
	}
	else
	{
		memcpy(g_play_hist_item.vdo_id.info_url, p_info->vdo_id.info_url,(strlen(p_info->vdo_id.info_url) + 1));
	}
        g_play_hist_item.vdo_id.type = p_info->vdo_id.type;
	//g_play_hist_item.vdo_id = p_info->vdo_id;
        g_play_hist_item.res_id = p_info->res_id;
	if (IPTV_ID_IQY ==  ui_iptv_dp_get_iptvId())
	{
       		 g_play_hist_item.vdo_id.type = p_info->b_single_page;
	}
        ...
        g_PHTimeTable[0].phy_pos = phy_pos;
        g_play_hist_cnt++;
    }
}
...
void db_plht_add_unique_item(play_hist_info *p_info)
{
	...
	OS_PRINTF("%s  %d \n",__FUNCTION__,__LINE__);
      //  if (temp_info.res_id == p_info->res_id &&(0 == memcmp(temp_info.vdo_id.qpId, p_info->vdo_id.qpId, strlen(p_info->vdo_id.qpId)))&&
	//	(0 == memcmp(temp_info.vdo_id.tvQid, p_info->vdo_id.qpId, strlen(p_info->vdo_id.tvQid)))&&
	//	(0 == memcmp(temp_info.vdo_id.sourceCode, p_info->vdo_id.qpId, strlen(p_info->vdo_id.sourceCode))))

		  if (0 == memcmp(temp_info.vdo_id.qpId, p_info->vdo_id.qpId, strlen(p_info->vdo_id.qpId))&&
		  	(0 == memcmp(temp_info.vdo_id.tvQid, p_info->vdo_id.tvQid, strlen(p_info->vdo_id.tvQid)))&&
		  	(0 == memcmp(temp_info.vdo_id.sourceCode, p_info->vdo_id.sourceCode, strlen(p_info->vdo_id.sourceCode))))
	        {
	       		 OS_PRINTF("%s  %d \n",__FUNCTION__,__LINE__);
	...
	{
		if(0 == memcmp(temp_info.vdo_id.info_url, p_info->vdo_id.info_url, strlen(p_info->vdo_id.info_url)+1))
	        {
	            db_plht_del_item_by_idx(i);
	            break;
	        }
