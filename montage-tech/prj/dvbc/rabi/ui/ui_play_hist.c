...
extern BOOL xm_db_plht_get_item_by_idx(u16 index, play_hist_info *p_info);
void xm_db_plht_del_all(void);
...
static RET_CODE ui_playhist_on_list_selected(control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
   ...
   {
	   memcpy(param.vdo_id.info_url, info.vdo_id.info_url, strlen(info.vdo_id.info_url)+1);
   }
    param.vdo_id.type = info.vdo_id.type;
    OS_PRINTF("%s ,%d, type[%d]\n",__FUNCTION__,__LINE__,param.vdo_id.type);
