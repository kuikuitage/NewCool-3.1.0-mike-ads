...
void ui_video_c_get_resolution(rect_size_t *p_rect)
{
    ...
    p_rect->w = status_info.video_disp_w;
    p_rect->h = status_info.video_disp_h;
...
void ui_video_c_get_insert_sub_by_idx(u16 idx, u16 *p_buf, u16 str_len)
{
    ...
    dvb_to_unicode(p_subt_info->subtitle[idx].title, str_len - 1, p_buf, str_len);
}
/*!
 * set subtitle id
 */
...
{
    rect_size_t     rect_size = {720, 140};
    RET_CODE        ret       = 0;
    void           *p_disp    = NULL;
    void           *p_gpe     = NULL;
    point_t         pos       = {0};
    rect_t          rect      = {0, 0, 720, 140};
...
RET_CODE ui_video_c_seek(s32 time_sec)
{
    ...
    //{
        ...
        ret = mul_file_play_seek(g_fp_c_priv.m_handle, time_sec);
    //}
...
RET_CODE ui_video_c_load_all_audio_track(void)
{
    ...
    audio_track_num = ui_video_c_get_audio_track_num();
...
RET_CODE ui_video_c_get_audio_track_lang_by_idx(int idx, u16 *p_buf, u16 len)
{
    ...
    if(NULL == g_fp_c_priv.p_audio_track_arr) //to fix bug 51363 
    {
        sprintf(asc_buf, "snd%d",  idx + 1);
        UI_VIDEO_PRINF("[%s]: asc:%s\n", __FUNCTION__, asc_buf);
        str_nasc2uni(asc_buf, p_buf, len);
    ...
    if((NULL != p_lang) && (0 != strlen(p_lang)) && (NULL != p_title) && (0 != strlen(p_lang)))
    {
        ...
        _ui_video_c_utf82uni(asc_buf, p_buf, len);
    } 
...
RET_CODE ui_video_c_play_by_file(u16 *p_file_name, u32 seek_play_time)
{
    ...
    //UI_VIDEO_PRINF("@@@file path=%s\n", p_asc_name);
    ret = mul_file_play_load_ex_media(g_fp_c_priv.m_handle, p_asc_name, VDO_PLAY_MODE_INVALID);
    if (ret == SUCCESS)
    {
...
RET_CODE ui_video_c_destroy(void)
{
    ...
    if(g_fp_c_priv.inited)
    {
        ...
        {
          //mtos_free(sg_preload_aud_buf);// if free wii reboot 
...
RET_CODE ui_video_c_create(u8 play_func)
{
    ...
    if (play_func == VIDEO_PLAY_FUNC_USB)
    {
      ...
      attr.vdec_start = VIDEO_FW_CFG_ADDR;
      attr.vdec_size = VIDEO_FW_CFG_SIZE;
      attr.audio_start = AUDIO_FW_CFG_ADDR;
      attr.audio_size = AUDIO_FW_CFG_SIZE;

      attr.v_mem_size = VIDEO_FILE_PLAY_SIZE;
      attr.v_mem_start = VIDEO_FILE_PLAY_ADDR;
      attr.vdec_policy = VDEC_QAULITY_AD_128M;
/*
...
*/
    ...
    else if (play_func == VIDEO_PLAY_FUNC_ONLINE)
    {
      ...
      attr.v_mem_size = VIDEO_FILE_PLAY_SIZE; // -4M for network play
      attr.v_mem_start = VIDEO_FILE_PLAY_ADDR;
