...
typedef enum
{
   IDC_IPTV_DESCNT_VARIETY_REVIEW_LIST = IDC_IPTV_DESCNT_VARIETY_RSC_TXT_LIST + 1,
   IDC_IPTV_DESCNT_VARIETY_REVIEW_DAY_LIST,
   IDC_IPTV_DESCNT_VARIETY_REVIEW_RSC_NUM_TEXT,
   IDC_IPTV_DESCNT_VARIETY_REVIEW_RSC_NUM_DYN,
}ui_variety_review_ctrl_id_t;
...
static iptv_ctrl_info_t iptv_ctrl_info[] = 
{
    ...
    {(u32)IDC_IPTV_DESCNT_VARIETY_REVIEW_RSC_NUM_TEXT,{"资源数:"}},
};
...
u16 iptv_des_variety_review_list_keymap(u16 key);
...
RET_CODE iptv_des_variety_review_list_proc(control_t *cont, u16 msg, u32 para1,u32 para2);
...
static void on_iptv_free_play_data(void)
{
    ...
    if(p_data != NULL)
    {
		...
		if(p_data->epg_list != NULL)
		{
			for(j=0; j<p_data->epg_day; j++)
			{
				if(p_data->epg_list[j].epgTitle)
					SY_FREE(p_data->epg_list[j].epgTitle);
			}
			SY_FREE(p_data->epg_list);
		}
...
static BOOL  get_iptv_is_fav(VDO_ID_t *video_id,u32 *fav_focus)
{
	...
	{
		 if(0 == memcmp(video_id->info_url, iptv_fav_info.iptv_fav_vdo_id.info_url,(strlen(video_id->info_url) + 1)))
                {
                    *fav_focus = i;
                    return TRUE;
                }
    ...
    if(p_data_iptv != NULL)
    {
        if(p_data_iptv->origin != NULL)
        {
            for (i = 0; i < size; i++)
            {
                if (i + start < cnt)
                {
                    if(p_data_iptv->play_origin_info[i+start+p_data_iptv->epg_count].urltitle == NULL)
                    {
                        ui_comm_dlg_open(&dlg_data);
                        return SUCCESS;
                    }
                    list_set_field_content_by_unistr(p_list, (u16)(start + i), 0, p_data_iptv->play_origin_info[i+start+p_data_iptv->epg_count].urltitle);
                }
            }
...
static RET_CODE iptv_desc_time_to_utc(const char *stime, utc_time_t *time)
{
  u32 i = 0,j=0;
  u32 tmp_value = 0;
  u32 tmp_date = 0;
  ...
  if((NULL == stime) || (NULL == time))
  {
      return ERR_FAILURE;
  }

  str_len = strlen(stime);
  DEBUG(UI_IPLAY_DESC,INFO,"stime = %s dest_len= %d\n",stime,str_len);
  if((0 == str_len))
  {
      return ERR_FAILURE;
  }
  ...
  for(i = 0,j=0; i < str_len; i++)
  {
    ...
    {
        tmp_value = stime[i] - '0';
        tmp_date = tmp_date * 10 + tmp_value;
    }
    else if(stime[i] == '-')
    {
		switch(j)
		...
		DEBUG(UI_IPLAY_DESC,INFO,"tmp_date = %d \n",tmp_date);
		tmp_date = 0;
		j++;
    }
  }
...
static RET_CODE on_iptv_variety_review_list_update(control_t* p_list, u16 start, u16 size, u32 context)
{
    u32 i;
	utc_time_t time;
	u8 week_day = 0;
	u16 week[7] = {IDS_MONDAY,IDS_TUESDAY,IDS_WEDNESDAY,IDS_THURSDAY,IDS_FRIDAY,IDS_SATURDAY,IDS_SUNDAY};
	u16 cnt = list_get_count(p_list);  
	al_iptv_play_origin_info_t *p_data_iptv = (al_iptv_play_origin_info_t *)on_ui_iptv_get_play_data();
    ...
    if(p_data_iptv != NULL)
    {
        for (i = 0; i < size; i++)
        {
            if (i + start < cnt)
            {
                if(p_data_iptv->epg_list[i+start].epgTitle != NULL)
                {
                    list_set_field_content_by_ascstr(p_list, (u16)(start + i), 0, p_data_iptv->epg_list[i+start].epgTitle);

					memset(&time,0,sizeof(utc_time_t));
					iptv_desc_time_to_utc((char *)p_data_iptv->epg_list[i+start].epgTitle, &time);
					week_day = date_to_weekday(&time);
					DEBUG(UI_IPLAY_DESC,INFO,"%d-%d-%d week_day = %d\n",time.year,time.month,time.day,week_day);
					list_set_field_content_by_strid(p_list, (u16)(start + i), 1, week[week_day]);
                }
            }
        }
        ctrl_paint_ctrl(p_list, TRUE);
...
static void create_iptv_desc_variety_review(control_t *p_des_cont)
{
    control_t *p_list = NULL;

    list_xstyle_t iptv_variety_review_list_field_fstyle =
    {
        FSI_GRAY,
        FSI_WHITE,
        FSI_WHITE,
    ...
    list_xstyle_t iptv_variety_review_list_field_rstyle =
    {
        RSI_IGNORE,
        RSI_IGNORE,
        RSI_IGNORE,
    ...
    list_xstyle_t iptv_variety_review_list_item_rstyle =
    {
        RSI_IGNORE,
        RSI_IGNORE,
        RSI_OTT_BUTTON_HL,
    ...
    list_field_attr_t iptv_variety_review_list_attr[IPTV_VARIETY_RSC_LIST_FIELD] =
    {   
        { LISTFIELD_TYPE_UNISTR | STL_LEFT | STL_VCENTER,
        480, 20, 0, &iptv_variety_review_list_field_rstyle,  &iptv_variety_review_list_field_fstyle},
    };
    //List
    control_t *p_ctrl_temp = NULL;

    if((p_ctrl_temp = ctrl_get_child_by_id(p_des_cont, IDC_IPTV_DESCNT_VARIETY_REVIEW_LIST)) != NULL)
    {
        DEBUG(UI_IPLAY_DESC,INFO,"destory create_iptv_rsc_txt_btn\n");
        ctrl_destroy_ctrl(p_ctrl_temp);
    }  

    p_list = ctrl_create_ctrl(CTRL_LIST, IDC_IPTV_DESCNT_VARIETY_REVIEW_LIST, 
                        IPTV_VARIETY_REVIEW_RSC_LIST_X, IPTV_VARIETY_REVIEW_RSC_LIST_Y, 
                        IPTV_VARIETY_REVIEW_RSC_LIST_W, IPTV_VARIETY_REVIEW_RSC_LIST_H, p_des_cont, 0);
    ctrl_set_rstyle(p_list, RSI_PBACK, RSI_PBACK, RSI_PBACK);
    ctrl_set_keymap(p_list, iptv_des_variety_list_keymap);
    ctrl_set_proc(p_list, iptv_des_variety_list_proc);
	ctrl_set_mrect(p_list, IPTV_VARIETY_REVIEW_RSC_LIST_MID_L,
							IPTV_VARIETY_REVIEW_RSC_LIST_MID_T,
    						IPTV_VARIETY_REVIEW_RSC_LIST_MID_W + IPTV_VARIETY_REVIEW_RSC_LIST_MID_L,
    						IPTV_VARIETY_REVIEW_RSC_LIST_MID_H + IPTV_VARIETY_REVIEW_RSC_LIST_MID_T);
    list_set_item_interval(p_list, IPTV_VARIETY_REVIEW_RSC_ITEM_VGAP);
    list_set_item_rstyle(p_list, &iptv_variety_review_list_item_rstyle);
    list_enable_select_mode(p_list, TRUE);
    list_set_select_mode(p_list, LIST_SINGLE_SELECT);
    list_set_count(p_list,0, IPTV_VARIETY_REVIEW_RSC_LIST_PAGE);
    list_set_field_count(p_list, IPTV_VARIETY_REVIEW_RSC_LIST_FIELD, IPTV_VARIETY_REVIEW_RSC_LIST_PAGE);
    list_set_update(p_list, on_iptv_variety_list_update, 0);
    list_set_focus_pos(p_list, 0);

    list_set_field_attr(p_list, 0, (u32)(iptv_variety_review_list_attr[0].attr), 
                        (u16)(iptv_variety_review_list_attr[0].width),
                        (u16)(iptv_variety_review_list_attr[0].left), 
                        (u8)(iptv_variety_review_list_attr[0].top));
    list_set_field_rect_style(p_list, 0, iptv_variety_review_list_attr[0].rstyle);
    list_set_field_font_style(p_list, 0, iptv_variety_review_list_attr[0].fstyle);

}

static void create_iptv_desc_variety_review_day(control_t *p_des_cont)
{
	u8 i;
    control_t *p_list = NULL;
    ...
    list_field_attr_t iptv_variety_review_list_attr[IPTV_VARIETY_REVIEW_DAY_RSC_LIST_FIELD] =
    {   
        { LISTFIELD_TYPE_UNISTR | STL_LEFT | STL_VCENTER,
        120, 20, 0, &iptv_variety_review_list_field_rstyle,  &iptv_variety_review_list_field_fstyle},
        { LISTFIELD_TYPE_STRID | STL_LEFT | STL_VCENTER,
        100, 160, 0, &iptv_variety_review_list_field_rstyle,  &iptv_variety_review_list_field_fstyle},        
    };
    //List
    control_t *p_ctrl_temp = NULL;

    if((p_ctrl_temp = ctrl_get_child_by_id(p_des_cont, IDC_IPTV_DESCNT_VARIETY_REVIEW_DAY_LIST)) != NULL)
    {
        DEBUG(UI_IPLAY_DESC,INFO,"destory create_iptv_rsc_txt_btn\n");
        ctrl_destroy_ctrl(p_ctrl_temp);
    }  

    p_list = ctrl_create_ctrl(CTRL_LIST, IDC_IPTV_DESCNT_VARIETY_REVIEW_DAY_LIST, 
                        IPTV_VARIETY_REVIEW_DAY_RSC_LIST_X, IPTV_VARIETY_REVIEW_DAY_RSC_LIST_Y+IPTV_VARIETY_REVIEW_RSC_ITEM_VGAP, 
                        IPTV_VARIETY_REVIEW_DAY_RSC_LIST_W, IPTV_VARIETY_REVIEW_DAY_RSC_LIST_H-IPTV_VARIETY_REVIEW_RSC_ITEM_VGAP, p_des_cont, 0);
    ctrl_set_rstyle(p_list, RSI_PBACK, RSI_PBACK, RSI_PBACK);
    ctrl_set_keymap(p_list, iptv_des_variety_review_list_keymap);
    ctrl_set_proc(p_list, iptv_des_variety_review_list_proc);
    list_set_item_interval(p_list, IPTV_VARIETY_REVIEW_RSC_ITEM_VGAP);
    list_set_item_rstyle(p_list, &iptv_variety_review_list_item_rstyle);
    list_enable_select_mode(p_list, TRUE);
    list_set_select_mode(p_list, LIST_SINGLE_SELECT);
    list_set_count(p_list,0, IPTV_VARIETY_REVIEW_DAY_RSC_LIST_PAGE);
    list_set_field_count(p_list, IPTV_VARIETY_REVIEW_DAY_RSC_LIST_FIELD, IPTV_VARIETY_REVIEW_DAY_RSC_LIST_PAGE);
    list_set_update(p_list, on_iptv_variety_review_list_update, 0);
    list_set_focus_pos(p_list, 0);

	for(i=0; i< IPTV_VARIETY_REVIEW_DAY_RSC_LIST_FIELD; i++)
	{
	    list_set_field_attr(p_list, i, (u32)(iptv_variety_review_list_attr[i].attr), 
	                        (u16)(iptv_variety_review_list_attr[i].width),
	                        (u16)(iptv_variety_review_list_attr[i].left), 
	                        (u8)(iptv_variety_review_list_attr[i].top));
	    list_set_field_rect_style(p_list, i, iptv_variety_review_list_attr[i].rstyle);
	    list_set_field_font_style(p_list, i, iptv_variety_review_list_attr[i].fstyle);
	}
	ctrl_default_proc(p_list,MSG_GETFOCUS, 0, 0);
}

static void create_iptv_desc_variety_review_rsc(control_t *p_des_cont)
{
    control_t *p_sub = NULL;
    u16 uni_str[16];

    u32 idc_id = (u32)IDC_IPTV_DESCNT_VARIETY_REVIEW_RSC_NUM_TEXT;
    on_ui_iptv_find_dis_str(idc_id,0,uni_str,sizeof(uni_str));

    p_sub = ctrl_create_ctrl(CTRL_TEXT, IDC_IPTV_DESCNT_VARIETY_REVIEW_RSC_NUM_TEXT, 
                      IPTV_VARIETY_REVIEW_RSC_TEXT_X, IPTV_VARIETY_REVIEW_RSC_TEXT_Y, 
                      IPTV_VARIETY_REVIEW_RSC_TEXT_W, IPTV_VARIETY_REVIEW_RSC_TEXT_H, 
                      p_des_cont, 0);
    ctrl_set_rstyle(p_sub, RSI_PBACK, RSI_PBACK, RSI_PBACK);
    text_set_align_type(p_sub, STL_LEFT | STL_CENTER);
    text_set_content_type(p_sub, TEXT_STRTYPE_UNICODE);
    ...
    //create description text field
    p_sub = ctrl_create_ctrl(CTRL_TEXT, IDC_IPTV_DESCNT_VARIETY_REVIEW_RSC_NUM_DYN, 
                      IPTV_VARIETY_REVIEW_RSC_PER_X, IPTV_VARIETY_REVIEW_RSC_PER_Y, 
                      IPTV_VARIETY_REVIEW_RSC_PER_W, IPTV_VARIETY_REVIEW_RSC_PER_H, 
                      p_des_cont, 0);
    ctrl_set_rstyle(p_sub, RSI_PBACK, RSI_PBACK, RSI_PBACK);
    text_set_align_type(p_sub, STL_LEFT | STL_CENTER);
    text_set_font_style(p_sub, FSI_WHITE, FSI_WHITE, FSI_WHITE);
...
static void create_iptv_desc_variety_review_container(control_t *p_menu)
{
    control_t *p_des_cont = NULL;
    ...
    create_iptv_desc_variety_review(p_des_cont);
    create_iptv_desc_variety_review_rsc(p_des_cont);
}

static void create_iptv_desc_variety_review_content(control_t *p_menu)
{
    control_t *p_des_cont_tent = NULL;
    ...
    //epg list
    create_iptv_desc_variety_review_day(p_des_cont_tent);
}
...
static u8  ui_iptv_choose_cate_id(ui_iptv_description_param_t *param)
{
    ...
    DEBUG(UI_IPLAY_DESC,INFO,"param->vdo_id.type = %d\n",param->vdo_id.type);
    if (param->vdo_id.type == IPTV_MOVIE)
        category = IPTV_MOVIE;
    else if (param->vdo_id.type == IPTV_VARIETY_REVIEW)
        category = IPTV_VARIETY_REVIEW;
    else
...
RET_CODE open_iptv_description(u32 para1, u32 para2)
{
	...
	if(IPTV_ID_IQY ==  ui_iptv_dp_get_iptvId())
		...
		ui_iptv_dat->category_id = p_param->vdo_id.type;
	else
	...
	//score
	if(ui_iptv_dat->category != IPTV_VARIETY_REVIEW)
	...
	else if(ui_iptv_dat->category == IPTV_VARIETY_REVIEW)
	{
		create_iptv_desc_variety_review_container(p_menu);
		create_iptv_desc_variety_review_content(p_menu);
	}
...
static RET_CODE on_iptv_des_vdo_info_arrive(control_t *p_ctrl, u16 msg,u32 para1, u32 para2)
{
	...
	if((IPTV_ID_XM == ui_iptv_dp_get_iptvId()) 
		&& (strcmp(p_data->vdo_id.qpId, ui_iptv_dat->video_id.qpId) != 0)
...
static RET_CODE on_iptv_des_series_info_url_arrive(control_t *p_ctrl, u16 msg,u32 para1, u32 para2)
{
	...
	control_t *p_des_variety_epg_list = NULL;
	u8 utf_str[8] = {0};
	u16 uni_str[32] = {0};
	...
	set_iptv_des_state(IPTV_DESC_URL_ARRIVE);
   	...
   	DEBUG(UI_IPLAY_DESC,INFO," ui_iptv_dat->url_arrive_semph[%d] \n",ui_iptv_dat->url_arrive_semph);
    if(ui_iptv_dat->url_arrive_semph > 1)
    {
        --ui_iptv_dat->url_arrive_semph;
        if(p_data_iptv->origin == NULL)
   ...
   --ui_iptv_dat->url_arrive_semph;
	...
	if(p_data == NULL)
	{
		DEBUG(UI_IPLAY_DESC,INFO,"data NULL \n");
		return ERR_FAILURE;
	}
...
*/
    ...
    if(ui_iptv_dat->category!=IPTV_VARIETY && ui_iptv_dat->category != IPTV_VARIETY_REVIEW)
    { 
	   	DEBUG(UI_IPLAY_DESC,INFO," ui_iptv_dat->category[%d] \n",ui_iptv_dat->category);
        p_des_cont = ctrl_get_child_by_id(p_ctrl, IDC_IPTV_DESCNT);
        ...
        if(ui_iptv_dat->category == IPTV_TV)
        {
			...
			if(ctrl_get_child_by_id(p_des_cont,IDC_IPTV_DESC_SERIES_NUM_MBOX) == NULL 
				|| ctrl_get_sts(ctrl_get_child_by_id(p_des_cont,IDC_IPTV_DESC_SERIES_NUM_MBOX)) == OBJ_STS_HIDE)
            {
    ...
    else if(ui_iptv_dat->category==IPTV_VARIETY_REVIEW)
    {
	   	DEBUG(UI_IPLAY_DESC,INFO," ui_iptv_dat->category[%d] \n",ui_iptv_dat->category);	
        p_des_variety_cont = ctrl_get_child_by_id(p_ctrl,IDC_IPTV_DESCNT);
        p_des_variety_cont_list = ctrl_get_child_by_id(p_des_variety_cont,IDC_IPTV_DESCNT_VARIETY_REVIEW_LIST);

		p_des_cont = ctrl_get_child_by_id(p_ctrl,IDC_IPTV_DES_VARIETY_TENT);
		p_des_variety_epg_list = ctrl_get_child_by_id(p_des_cont,IDC_IPTV_DESCNT_VARIETY_REVIEW_DAY_LIST);
	}
	...
	p_data_iptv->epg_day = p_data->epgDay;
	p_data_iptv->epg_list =  (al_iptv_play_origin_epg_list_t*)SY_MALLOC(p_data->epgDay*(sizeof(al_iptv_play_origin_epg_list_t)));
	memset(p_data_iptv->epg_list,0, p_data->epgDay*(sizeof(al_iptv_play_origin_epg_list_t)));	
	DEBUG(UI_IPLAY_DESC,INFO,"epg_day = %d \n",p_data->epgDay);
	if(p_data->epgDay && (ui_iptv_dat->category == IPTV_VARIETY_REVIEW))
	{
		for(i=0; i<p_data->epgDay; i++)
		{
			p_data_iptv->epg_list[i].count = p_data->epgList[i].count;

			if(p_data->epgList[i].epgTitle != NULL)
			{
				p_data_iptv->epg_list[i].epgTitle=  (u8*)SY_MALLOC(strlen(p_data->epgList[i].epgTitle)+1);
				memset(p_data_iptv->epg_list[i].epgTitle,0, strlen(p_data->epgList[i].epgTitle)+1);
				strcpy(p_data_iptv->epg_list[i].epgTitle, p_data->epgList[i].epgTitle);
			}
			DEBUG(UI_IPLAY_DESC,INFO,"epg list [%d] = %d,%s\n",i,strlen(p_data_iptv->epg_list[i].epgTitle),p_data_iptv->epg_list[i].epgTitle);
		}
	}
	DEBUG(UI_IPLAY_DESC,INFO,"epg_dat = %d \n",p_data->epgDay);
    ...
    for(j = ui_iptv_dat->url_start[0],
            ...
            j++,i++)
    {
		p_data_iptv->play_origin_info[j].id = p_data->urlList[i].id;
		if(ui_iptv_dat->category == IPTV_TV)
		{
			;
		}
		...
		if(ui_iptv_dat->category == IPTV_VARIETY
			||ui_iptv_dat->category == IPTV_VARIETY_REVIEW)
		{
			if(p_data->urlList[i].urltitle != NULL)
			{
				...
				p_data_iptv->play_origin_info[j].urltitle=  (u16*)SY_MALLOC(uni_strlen(p_data->urlList[i].urltitle) * 2 + 2);
				memset(p_data_iptv->play_origin_info[j].urltitle,0, uni_strlen(p_data->urlList[i].urltitle) * 2 + 2);
    ...
    shell_sort(p_data_iptv[orgin_focus].play_origin_info, p_data_iptv[orgin_focus].url_count,
      ...
      {  
		origin_txt = ctrl_get_child_by_id(p_des_variety_cont,IDC_IPTV_DESCNT_VARIETY_RSC_NUM_TEXT);
		if(origin_txt)
		{
		...
		ui_iptv_dat->iptv_entf == FALSE)
		{
			...
			for (i = 0; i < IPTV_VARIETY_RSC_LIST_PAGE; i++)
			{
				...
				{
					list_set_field_content_by_unistr(p_des_variety_cont_list, (u16) i, 0, p_data_iptv->play_origin_info[i].urltitle);
              }
            }
	...
	{
		origin_txt = ctrl_get_child_by_id(p_des_variety_cont,IDC_IPTV_DESCNT_VARIETY_REVIEW_RSC_NUM_TEXT);
		if(origin_txt)
		{
			if(ctrl_get_sts(origin_txt) == OBJ_STS_HIDE)
			{
		...
		origin_txt = ctrl_get_child_by_id(p_des_variety_cont,IDC_IPTV_DESCNT_VARIETY_REVIEW_RSC_NUM_DYN);
		if(origin_txt)
		{
			if(ctrl_get_sts(origin_txt) == OBJ_STS_HIDE)
			{   
				p_data_iptv->epg_force = 0;
				sprintf(utf_str,"%d/%d",(int)1,(int)p_data->epgList[0].count);
				convert_gb2312_chinese_asc2unistr(utf_str, uni_str, sizeof(uni_str));
				text_set_content_by_unistr(origin_txt, uni_str);
				ctrl_set_sts(origin_txt, OBJ_STS_SHOW);
			}
		...
		if(list_get_field_content(p_des_variety_cont_list, 0,0) == 0&&ui_iptv_dat->iptv_entf == FALSE)
		{
			p_data_iptv->epg_count = 0;
			list_set_count(p_des_variety_cont_list,p_data->epgList[0].count, IPTV_VARIETY_RSC_LIST_PAGE);   
			on_iptv_variety_list_update(p_des_variety_cont_list,0,p_data->epgList[0].count,0);
            i = list_get_focus_pos(p_des_variety_cont_list);
            list_set_focus_pos(p_des_variety_cont_list, (i == 0xFFFF)?0:i);
            ui_iptv_dat->iptv_entf = TRUE;

			list_set_count(p_des_variety_epg_list,p_data->epgDay, IPTV_VARIETY_REVIEW_DAY_RSC_LIST_PAGE); 
			on_iptv_variety_review_list_update(p_des_variety_epg_list,0,p_data->epgDay,0);
			i = list_get_focus_pos(p_des_variety_epg_list);
			list_set_focus_pos(p_des_variety_epg_list, (i == 0xFFFF)?0:i);
        }

        ctrl_paint_ctrl(p_des_variety_cont, TRUE);
		ctrl_paint_ctrl(p_des_variety_epg_list, TRUE);
	}
...
static RET_CODE on_iptv_des_pic_update(control_t *p_ctrl, u16 msg,u32 para1, u32 para2)
{
    ...
    else if(p_img_url == NULL && ui_iptv_dat->category != IPTV_VARIETY && ui_iptv_dat->category != IPTV_VARIETY_REVIEW)
    {
...
static RET_CODE on_iptv_pic_draw_end(control_t *p_cont, u16 msg, u32 para1, u32 para2)
{
 ...
 if(para1 == 0)
 {
    ...
    pic_stop();
    if(ui_iptv_dat->playbar_is_back == TRUE && (ui_iptv_dat->category != IPTV_VARIETY && ui_iptv_dat->category != IPTV_VARIETY_REVIEW))
    {
        on_iptv_update_rec_pic(p_ctrl);  
    }
     return ERR_FAILURE;
  ...
  if(ui_iptv_dat->identify != para1)
  {
   ...
   if(((para1 >> 16) & 0xFFFF) == 0xFFFF)
   {
    ...
    else
    {
        ...
        if(ui_iptv_dat->playbar_is_back == TRUE 
			&& (ui_iptv_dat->category != IPTV_VARIETY && ui_iptv_dat->category != IPTV_VARIETY_REVIEW))
        {
            on_iptv_update_rec_pic(p_ctrl);  
        }
        return ERR_FAILURE;
...
static RET_CODE on_iptv_variety_collect_focus_move(control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
    ...
    switch(msg)
    {
		...
		{
			   memcpy(iptv_info.iptv_fav_vdo_id.info_url, ui_iptv_dat->video_id.info_url, strlen(ui_iptv_dat->video_id.info_url)+1);
		}
...
static void ui_iptvdes_continue_to_play(void)
{
    ...
    if(p_data->play_origin_info == NULL)
    {
     ...
     DEBUG(UI_IPLAY_DESC,INFO,"play tvQid[%s],vid[%s]\r\n",p_data->play_origin_info[ui_iptv_dat->episode_num].id.tvQid,
	 			p_data->play_origin_info[ui_iptv_dat->episode_num].id.vid);
   
    if(IPTV_ID_IQY ==  ui_iptv_dp_get_iptvId()
		&& (p_data->play_origin_info[ui_iptv_dat->episode_num].id.tvQid[0] == 0
			||p_data->play_origin_info[ui_iptv_dat->episode_num].id.vid[0] == 0))
    {
      return;
    }
    DEBUG(UI_IPLAY_DESC,INFO,"\r\n");
    ui_iptv_dat->video_id =  p_data->play_origin_info[ui_iptv_dat->episode_num].id;
...
static RET_CODE on_iptv_des_video_rsc_begin_play(control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
	...
	{ 
		if(p_data != NULL)
		{
			if(p_data->play_origin_info != NULL)
			{
				DEBUG(UI_IPLAY_DESC,INFO,"play tvqid[%s],vid[%s]\r\n",p_data->play_origin_info[0].id.tvQid,p_data->play_origin_info[0].id.vid);
				if((IPTV_ID_XM ==  ui_iptv_dp_get_iptvId()
					&& (p_data->play_origin_info[0].id.tvQid[0] == 0 && p_data->play_origin_info[0].id.vid[0] == 0))
						|| (IPTV_ID_IQY ==  ui_iptv_dp_get_iptvId()))
				{
					...
					if (IPTV_ID_IQY ==  ui_iptv_dp_get_iptvId())
					{
						...
						{
							strcat((u8 *)key, p_data->play_origin_info[ui_iptv_dat->episode_num].id.vid);
							strcat((u8 *)key, p_data->play_origin_info[ui_iptv_dat->episode_num].id.tvQid);
						}
					}
	...
	else if(mbox_get_focus(p_ctrl) == 1)
	{
		...
		else
		{
			...
			{
				memcpy(iptv_fav_info.iptv_fav_vdo_id.info_url, ui_iptv_dat->video_id.info_url, strlen(ui_iptv_dat->video_id.info_url)+1);
			}
...
static RET_CODE on_iptv_desc_series_select(control_t *p_ctrl, u16 msg,u32 para1, u32 para2)
{
	...
	if(ui_iptv_dat->category == IPTV_VARIETY_REVIEW)
		ui_iptv_dat->episode_num = focus + p_data->epg_count;
	else
  ...
  switch(msg)
  {
   ...
   if(old_page!=new_page)
   {
    ...
    case MSG_SELECT:
	   ...
	   p_des_cont = ctrl_get_child_by_id(ctrl_get_child_by_id(ctrl_get_parent(p_cont), \
                              IDC_IPTV_DESCNT),
                              IDC_IPTV_DESCNT_MOVIE_RSC_MBOX);
          ctrl_process_msg(p_cont, MSG_LOSTFOCUS, 0, 0);
	   ctrl_process_msg(p_des_cont, MSG_GETFOCUS, 0, 0);
    break;
...
static void ui_iptvdesc_update_rsc_num(u16 list_focus)
{
	u8 utf_str[8] = {0};
	u16 uni_str[32] = {0};
	control_t *p_cont = NULL;
	control_t *p_rsc_num = NULL;
	al_iptv_play_origin_info_t *p_data = (al_iptv_play_origin_info_t *)on_ui_iptv_get_play_data();
	...
	p_cont = ui_comm_root_get_ctrl(ROOT_ID_IPTV_DESCRIPTION,IDC_IPTV_DESCNT);
	if(IPTV_VARIETY_REVIEW== ui_iptv_dat->category)
	{
		if(p_data->epg_list == NULL)
			return;

		p_rsc_num = ctrl_get_child_by_id(p_cont,IDC_IPTV_DESCNT_VARIETY_REVIEW_RSC_NUM_DYN);
		sprintf(utf_str,"%d/%d",(int)list_focus + 1,(int)p_data->epg_list[p_data->epg_force].count);
	}
	else
	{
		p_rsc_num = ctrl_get_child_by_id(p_cont,IDC_IPTV_DESCNT_VARIETY_RSC_NUM_DYN);
		sprintf(utf_str,"%d/%d",(int)list_focus + 1,(int)p_data->url_count);

	}

    convert_gb2312_chinese_asc2unistr(utf_str, uni_str, sizeof(uni_str));
    text_set_content_by_unistr(p_rsc_num, uni_str);
    ctrl_paint_ctrl(p_rsc_num, TRUE);	
}
...
static RET_CODE on_iptv_variety_list_focus_move(control_t *p_ctrl, u16 msg,u32 para1, u32 para2)
{
	...
	if(IPTV_VARIETY == ui_iptv_dat->category)
	{
    	switch(msg)
	    {
	        case MSG_FOCUS_LEFT:
	...
	else if(IPTV_VARIETY_REVIEW== ui_iptv_dat->category)
	{
		switch(msg)
		{
			case MSG_FOCUS_LEFT:
			case MSG_FOCUS_RIGHT:
                ...
                p_new_focus_ctrl = ctrl_get_child_by_id(p_cont,IDC_IPTV_DESCNT_VARIETY_REVIEW_DAY_LIST);
                if(p_new_focus_ctrl == NULL)
                    return SUCCESS;
	        ...
	        case MSG_PAGE_UP:
				...
				ui_iptvdesc_update_rsc_num(list_focus);				
				break;
	        case MSG_SELECT:
	            list_focus = list_get_focus_pos(p_ctrl);
	            ui_iptv_dat->episode_num = list_focus + p_data->epg_count;
...
static RET_CODE on_iptv_variety_review_day_list_focus_move(control_t *p_ctrl, u16 msg,u32 para1, u32 para2)
{
	control_t *p_cont, *p_new_focus_ctrl;
	u16 list_focus,i=0;
	al_iptv_play_origin_info_t *p_data = (al_iptv_play_origin_info_t *)on_ui_iptv_get_play_data();
	...
	switch(msg)
	{
		...
		case MSG_SELECT:
            ...
            p_new_focus_ctrl = ctrl_get_child_by_id(p_cont,IDC_IPTV_DESCNT_VARIETY_REVIEW_LIST);
            if(p_new_focus_ctrl == NULL)
                return SUCCESS;
        ...
        case MSG_PAGE_UP:
			...
			p_data->epg_force = list_focus;
			if(list_focus >0)
			{
				p_data->epg_count = 0;
				for(i=0; i<list_focus; i++)
					p_data->epg_count += p_data->epg_list[i].count;
			}
			else
			{
				p_data->epg_count = 0;
			...
			list_set_count(p_new_focus_ctrl,p_data->epg_list[list_focus].count, IPTV_VARIETY_RSC_LIST_PAGE);
			on_iptv_variety_list_update(p_new_focus_ctrl,0,p_data->epg_list[list_focus].count,0);
            list_set_focus_pos(p_new_focus_ctrl, 0);
			ctrl_paint_ctrl(p_new_focus_ctrl, TRUE);
			ui_iptvdesc_update_rsc_num(0);
...
u16 *ui_iptvdesc_get_episode_name(u8 category, u16 episode_num)
{
    ...
    if(category != IPTV_VARIETY && category != IPTV_VARIETY_REVIEW)
    {
        p_ctrl = ui_comm_root_get_ctrl(ROOT_ID_IPTV_DESCRIPTION,IDC_IPTV_DESC_CONT);
        p_des_tent = ctrl_get_child_by_id(p_ctrl, IDC_IPTV_DESC_TITLE);
        if(p_des_tent == NULL)
    ...
    else if(category == IPTV_VARIETY || category == IPTV_VARIETY_REVIEW)
    {
      al_iptv_play_origin_info_t *p_data_iptv = (al_iptv_play_origin_info_t *)on_ui_iptv_get_play_data();
      if(p_data_iptv != NULL)
      {
...
void ui_iptvdesc_get_info_url(u8  category, u16 episode_num,VDO_ID_t *vdo_id)
{
    ...
    if(category != IPTV_VARIETY && category != IPTV_VARIETY_REVIEW)
    {
        ...
        {
            *vdo_id = p_data->play_origin_info[0].id;
        }
        else
        {
            *vdo_id = p_data->play_origin_info[episode_num].id;
        }
    }
    else if(category == IPTV_VARIETY || category == IPTV_VARIETY_REVIEW)
    {
...
BEGIN_KEYMAP(iptv_des_variety_review_list_keymap, NULL)
	ON_EVENT(V_KEY_MENU, MSG_EXIT)
	ON_EVENT(V_KEY_BACK, MSG_EXIT)
	ON_EVENT(V_KEY_CANCEL, MSG_EXIT_ALL)
	ON_EVENT(V_KEY_LEFT, MSG_FOCUS_LEFT)
	...
	ON_EVENT(V_KEY_PAGE_DOWN, MSG_PAGE_DOWN)
	ON_EVENT(V_KEY_OK, MSG_SELECT)
END_KEYMAP(iptv_des_variety_review_list_keymap, NULL)

BEGIN_MSGPROC(iptv_des_variety_review_list_proc, list_class_proc)
	ON_COMMAND(MSG_FOCUS_LEFT, on_iptv_variety_review_day_list_focus_move)
	ON_COMMAND(MSG_FOCUS_RIGHT, on_iptv_variety_review_day_list_focus_move)
	ON_COMMAND(MSG_FOCUS_DOWN, on_iptv_variety_review_day_list_focus_move)

	ON_COMMAND(MSG_PAGE_UP, on_iptv_variety_review_day_list_focus_move)
	ON_COMMAND(MSG_PAGE_DOWN, on_iptv_variety_review_day_list_focus_move)

	ON_COMMAND(MSG_FOCUS_UP, on_iptv_variety_review_day_list_focus_move)
	ON_COMMAND(MSG_SELECT, on_iptv_variety_review_day_list_focus_move)
END_MSGPROC(iptv_des_variety_review_list_proc, list_class_proc)
