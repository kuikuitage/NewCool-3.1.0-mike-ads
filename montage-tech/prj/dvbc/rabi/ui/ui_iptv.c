...
static BOOL ui_init_app_data(u32 iptvType)
{
    if (g_pIptv)
    {
        ui_release_app_data();
	...
	g_pIptv->iptv_type = iptvType;
	switch(g_pIptv->iptv_type)
	{
		case IPTV_ID_IQY:
			g_pIptv->cat_extra_num   = IPTV_CATLIST_ALL + 1;
			g_pIptv->cat_fault_fcous = IPTV_CATLIST_ALL;
			break;
		case IPTV_ID_XM:
			g_pIptv->cat_extra_num   = IPTV_CATLIST_SEARCH + 1;
			g_pIptv->cat_fault_fcous = IPTV_CATLIST_SEARCH + 1;
			break;
		default:
			break;
	}
 ...
 */
static u16 ui_iptv_update_catlist_iqy_extern(control_t *p_catlist, u16 start)
{
	u16 *p_unistr;
	u16 i;
...
static u16 ui_iptv_update_catlist_xinmei_extern(control_t *p_catlist, u16 start)
{
	u16 *p_unistr;
	u16 i;
...
static RET_CODE ui_iptv_update_catlist(control_t *p_catlist, u16 start, u16 size, u32 context)
{
	...
	{
		case IPTV_ID_IQY:
			i = ui_iptv_update_catlist_iqy_extern(p_catlist, start);
			break;
		case IPTV_ID_XM:
			i = ui_iptv_update_catlist_xinmei_extern(p_catlist, start);
			break;			
		default:
			i = start;
			break;
    ...
    for (; i < start + size && i < cnt; i++){
        list_set_field_content_by_unistr(p_catlist, i, 0, g_pIptv->catList[i - g_pIptv->cat_extra_num].name);
    	DEBUG(UI_IPTV,INFO,"@@@ui_iptv_update_catlist name[%d] = %s\n",i, g_pIptv->catList[i - g_pIptv->cat_extra_num].name);
...
/* --------------------- Initialize:  ---------------------*/
extern void db_plht_sort_time_table(void);
...
static STATEID StaIptv_initialize_on_newrescatgry_arrive(control_t *ctrl, u16 msg, u32 para1, u32 para2)
{
    ...
    if (p_data->total_res > 0 && p_data->resList != NULL)
    {
        ...
        list_set_count(g_pIptv->hCatList, g_pIptv->total_cat + g_pIptv->cat_extra_num, IPTV_CATLIST_PAGE_SIZE);
        if (g_pIptv->total_cat > 0)
        {
            list_set_focus_pos(g_pIptv->hCatList, g_pIptv->curCatListIdx + g_pIptv->cat_fault_fcous);
            list_select_item(g_pIptv->hCatList, g_pIptv->curCatListIdx + g_pIptv->cat_fault_fcous);
...
static STATEID ui_iptv_filter_get_video_list(control_t *ctrl)
{
	...
	if((IPTV_CATLIST_ALL == list_get_focus_pos(g_pIptv->hCatList))&&(g_pIptv->iptv_type == IPTV_ID_IQY))
	{
...
static STATEID StaIptv_resource_on_catlist_selected(control_t *ctrl, u16 msg, u32 para1, u32 para2)
{
	...
	switch(focus)
	{
		...
		case IPTV_CATLIST_FILTER:
			if(g_pIptv->iptv_type == IPTV_ID_IQY)
			{
	...
	g_pIptv->curCatListIdx = focus - g_pIptv->cat_extra_num;
	g_pIptv->catFilter_flag = IPTV_FLAG_CAT;
	ctrl_paint_ctrl(ctrl, TRUE);
...
static void SenIptv_ResVdoReq(void)
{
	...
	if((g_pIptv->catFilter_flag == IPTV_FLAG_FILTER)&&(IPTV_ID_IQY == g_pIptv->iptv_type))
	{
    ...
    if (g_pIptv->catList)
    {
        	ui_iptv_get_video_list(g_pIptv->resList[g_pIptv->curResListIdx].res_id, 
			     g_pIptv->catList[g_pIptv->curCatListIdx].cat_id,
...
static STATEID StaIptv_resource_on_newpagevdo_arrive(control_t *ctrl, u16 msg, u32 para1, u32 para2)
{
	...
	DEBUG(UI_IPTV,INFO,"type[%d]\n",p_vdoList[i].vdo_id.type);
...
static STATEID StaIptv_resource_on_vdolist_selected(control_t *ctrl, u16 msg, u32 para1, u32 para2)
{
    ...
    if((g_pIptv->vdoList)&&(get_iptv_des_state() == IPTV_DESC_DESTORY))
    {
		...
		memcpy(p_param.vdo_id.info_url,g_pIptv->vdoList[g_pIptv->curVdoListIdx].vdo_id.info_url,strlen(g_pIptv->vdoList[g_pIptv->curVdoListIdx].vdo_id.info_url)+1);
