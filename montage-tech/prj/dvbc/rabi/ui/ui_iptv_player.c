...
static void ui_iptvplayer_init_episode_info(void)
{
    ...
    {
        uni_strcpy(g_pIptvPlayer->key, g_pIptvPlayer->pg_name);
        strcat((u8 *)g_pIptvPlayer->key, g_pIptvPlayer->vdo_id.vid);
        strcat((u8 *)g_pIptvPlayer->key, g_pIptvPlayer->vdo_id.tvQid);
		...
		|| g_pIptvPlayer->category == IPTV_VARIETY_REVIEW)
    {
...
static RET_CODE ui_iptvplayer_on_formatlist_select(control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
    ...
    ui_iptv_get_play_url(g_pIptvPlayer->formatList[g_pIptvPlayer->format_index].id, 
    			&g_pIptvPlayer->vdo_id,g_pIptvPlayer->episode_num);
...
static RET_CODE ui_iptvplayer_on_source_format_arrive(control_t *p_cont, u16 msg, u32 para1, u32 para2)
{
	...
	g_pIptvPlayer->vdo_id.tvQid,g_pIptvPlayer->vdo_id.vid);
	ui_iptv_get_play_url(g_pIptvPlayer->formatList[g_pIptvPlayer->format_index].id, 
							&g_pIptvPlayer->vdo_id,g_pIptvPlayer->episode_num);
...
static RET_CODE ui_iptvplayer_on_play_url_arrive(control_t *p_cont, u16 msg, u32 para1, u32 para2)
{
    ...
    if (p_data->total_url > 0 && p_data->pp_urlList != NULL)
    {
        ...
        g_pIptvPlayer->pp_urlList = (al_iptv_play_url_info_t*)SY_CALLOC(1,p_data->total_url * sizeof(al_iptv_play_url_info_t));
		...
		memcpy(g_pIptvPlayer->pp_urlList[i].playUrl, p_data->pp_urlList[i].playUrl,sizeof(g_pIptvPlayer->pp_urlList[i].playUrl));		
        }
...
extern void xm_db_plht_add_unique_item(play_hist_info *p_info);
...
if( g_pIptvPlayer->category != IPTV_VARIETY_REVIEW)
{
   if (IPTV_ID_IQY ==	ui_iptv_dp_get_iptvId())
   {
	    ...
	    info.vdo_id.type = g_pIptvPlayer->category;
   }
   else
   {
	//   memcpy(info.vdo_id.info_url, g_pIptvPlayer->vdo_id.info_url, (strlen(g_pIptvPlayer->vdo_id.info_url) + 1));
		info.vdo_id = *xingmei_get_current_video_id();
   }   
    DEBUG(UI_PLAYER,INFO,"type[%d]\n",info.vdo_id.type);	
...
{
    DEBUG(UI_PLAYER,INFO,"espisode num = %d,total = %d\n",g_pIptvPlayer->episode_num,g_pIptvPlayer->total_episode);
    if (g_pIptvPlayer->episode_num < g_pIptvPlayer->total_episode - 1)
    {
		...
		|| (g_pIptvPlayer->vdo_id.vid[0]!=0 && g_pIptvPlayer->vdo_id.tvQid[0]!=0))
        {
            g_pIptvPlayer->play_time = 0;
            ui_iptvplayer_open_dlg();
            g_pIptvPlayer->b_showloading = TRUE;
            ui_iptv_get_play_format(&g_pIptvPlayer->vdo_id, g_pIptvPlayer->episode_num);
        }
    }
    else
    {
...
/////////////////////////////////////////////////////////////////////////////
    if (((IPTV_ID_IQY ==  ui_iptv_dp_get_iptvId()) && g_pIptvPlayer->vdo_id.vid[0] != 0 && g_pIptvPlayer->vdo_id.tvQid[0] != 0)
	||IPTV_ID_XM==  ui_iptv_dp_get_iptvId())
    {
        ui_iptvplayer_open_dlg();
        g_pIptvPlayer->b_showloading = TRUE;
        g_pIptvPlayer->b_showAdTime = FALSE;
