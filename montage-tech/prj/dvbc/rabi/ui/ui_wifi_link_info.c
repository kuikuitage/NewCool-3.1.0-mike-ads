...
static BOOL is_repeat_connect_wifi = FALSE;
...
static wifi_info_t p_cur_wifi_info = {{0},{0}};
...
extern void ui_wifi_info_search(void);
extern void set_wifi_select_essid(u8 *essid);
...
static u16 wifi_get_connected_wifi_focus(u8 *name)
{
#define WIFI_MAC_COUNT  (100)
	...
	if(SUCCESS == dev_io_ctrl(p_wifi_dev, ETH_WIFI_LINK_INFO, (u32)&wif_sts_info))
	{
		if(WIFI_CONNECTED == wif_sts_info.connect_sts)
		{
			strcpy(name, wif_sts_info.essid);
	...
	for(i=0; i<WIFI_MAC_COUNT; i++)
	{
		...
		DEBUG(MAIN,INFO,"ap_info.essid = %s name = %s \n",ap_info.essid,name);
		if((strcmp(name, ap_info.essid) == 0) 
			&& (strlen(name) == strlen(ap_info.essid)))
		{
			connected_focus =  i;
			break;
		}
...
void set_connecting_wifi_info(wifi_info_t temp)
{
  p_cur_wifi_info = temp;
}

wifi_info_t get_connecting_wifi_info(void)
{
  return p_cur_wifi_info;
}
    ...
    switch(i)
    {
      case 0:
        ...
        if(!is_add_wifi)
        {
          ...
          ui_comm_t9_v2_edit_set_content_by_unistr(p_ctrl[i], uni_str);
...
static void create_wifi_help_bar(control_t *p_menu)
{
  ...
  if(strcmp(p_temp_wifi_info.essid, p_wifi_info.essid) == 0 && strcmp(p_wifi_info.essid, ap_info.essid) == 0 && is_connect_curn && !is_add_wifi)
  {
...
RET_CODE open_wifi_link_info(u32 para1, u32 para2)
{
  ...
  is_repeat_connect_wifi = FALSE;
  #ifndef WIN32
  wifi_ap_info_get(p_wifi_dev,list_focus,&ap_info);
  #endif

  set_wifi_select_essid(ap_info.essid);
  sys_status_get_wifi_info(&p_wifi_info,ap_info.essid);
  DEBUG(DBG, INFO, "essid:%s, key:%s\n",  p_wifi_info.essid, p_wifi_info.key);
...
static void paint_other_wifi_not_connect()
{
  ...
  p_list = ui_comm_right_root_get_ctrl(ROOT_ID_WIFI, 8);//IDC_WIFI_LIST
...
void do_cmd_to_conn_wifi()
{
  ...
  if(wifi_encrypt_type == IW_ENC_NONE)
  {
   ...
   DEBUG(MAIN, INFO, "essid:%s, key:%s	p_temp_wifi_info.essid= %s \n",    p_wifi_info.essid, p_wifi_info.key,p_temp_wifi_info.essid);
   //sys_status_save_wifi_password(&p_wifi_info);
   //sys_status_save();
   set_connecting_wifi_info(p_wifi_info);
...
void disconnect_curn_wifi(ethernet_device_t * p_dev)
{
  comm_dlg_data_t dis_wifi_dlg_data =
  {
    ROOT_ID_WIFI,
    DLG_FOR_SHOW | DLG_STR_MODE_STATIC,
    150+COMM_DLG_X, 41+COMM_DLG_Y, COMM_DLG_W, COMM_DLG_H,
...
static RET_CODE on_connect_wifi(control_t *p_cont, u16 msg, u32 para1, u32 para2)
{
  ...
  {
    #ifndef WIN32
    is_repeat_connect_wifi = TRUE;
	
    if(strcmp(p_temp_wifi_info.essid, ap_info.essid) == 0 && is_connect_curn && !is_add_wifi)
    {
	...
	fw_tmr_create(ROOT_ID_WIFI_LINK_INFO, MSG_EXIT, 60*SECOND, FALSE);
...
{
  u16 cur_focus;
  u8  essid[MAX_SSID_LEN+1] = {0};

  cur_focus = wifi_get_connected_wifi_focus(essid);
  {
	list_focus = cur_focus;
	p_wifi_info = get_connecting_wifi_info();
	DEBUG(MAIN, INFO, "cur_focus = %d essid = %s \n",cur_focus, essid);
	DEBUG(MAIN, INFO, "essid = %s key = %s \n",p_wifi_info.essid, p_wifi_info.key);
  }
  
  OS_PRINTF("###connect wifi successfully  p_wifi_info.essid = %s####\n",p_wifi_info.essid);
                             ...
                             u32 para2)
{
  ...
  if(is_repeat_connect_wifi == TRUE)
  {
	  return SUCCESS;
  }
...
static RET_CODE on_wifi_link_info_destory(control_t *p_cont,u16 msg, u32 para1, u32 para2)
{
	if(is_repeat_connect_wifi == TRUE)
		ui_wifi_info_search();
	...
	fw_tmr_destroy(ROOT_ID_WIFI_LINK_INFO, MSG_EXIT);
	return ERR_NOFEATURE;
}
...
//  ON_EVENT(V_KEY_BLUE, MSG_EXIT)
  ON_EVENT(V_KEY_HOT_XSAVE, MSG_RED)
...
BEGIN_MSGPROC(wifi_link_info_cont_proc, cont_class_proc)
  ...
  ON_COMMAND(MSG_DESTROY, on_wifi_link_info_destory)
