...
typedef struct VDO_ID_S{
	...
	int 	 cat_id;
	int     program_id;
	char  info_url[IPTV_URL_LEN];
	
	char    qpId[IPTV_STRING_ID_LEN];                 	//专辑奇谱（视频/专辑/来源）ID for iqy
	char    tvQid[IPTV_STRING_ID_LEN];                	//剧集奇谱ID for iqy
	char    vid[IPTV_STRING_ID_LEN];                	//剧集奇谱ID for iqy
	
	char    sourceCode[IPTV_STRING_SHORT];   			//来源code
	ui_iptv_mode_t     type;                               		//专辑类型（0：视频，1：专辑）调用此接口传进来的那个qpId的数据类型，不是返回结果中的qpId的数据类型
