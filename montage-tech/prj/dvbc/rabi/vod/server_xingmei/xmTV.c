...
void XM_update_ProgramDetailRequest(char *url)
{
	static char buf[64] = {0};
	utc_time_t start_time,end_time;
	unsigned long mk_time;
	struct tm * tm = NULL;

	strtok(url,"&");
	DEBUG(XINGMEI,INFO,"url [%s]\n",url);

	sys_status_get_utc_time(&end_time);
	mk_time = _mktime(end_time.year,end_time.month,end_time.day,end_time.hour,end_time.minute,end_time.second);
	mk_time -= 7*24*3600;
	mk_time = ((mk_time + (1*24*3600)-1)/(1*24*3600))*(1*24*3600);
	tm = localtime(&mk_time);
    start_time.year 	= tm->tm_year + 1900;
    start_time.month 	= tm->tm_mon + 1;
    start_time.day 	= tm->tm_mday;
    start_time.hour 	= tm->tm_hour;
    start_time.minute = tm->tm_min;
    start_time.second = tm->tm_sec;

	sprintf(buf,"&start=%d-%02d-%02d %02d:%02d:%02d&end=%d-%02d-%02d %02d:%02d:%02d",
		start_time.year,start_time.month,start_time.day,
		start_time.hour,start_time.minute,start_time.second,
		end_time.year,end_time.month,end_time.day,
		end_time.hour,end_time.minute,end_time.second);
	DEBUG(XINGMEI,INFO,"&start=%d-%02d-%02d %02d:%02d:%02d&end=%d-%02d-%02d %02d:%02d:%02d\n",
		start_time.year,start_time.month,start_time.day,
		start_time.hour,start_time.minute,start_time.second,
		end_time.year,end_time.month,end_time.day,
		end_time.hour,end_time.minute,end_time.second);
...
char *XM_AttachUserToken(char * domain)
{
	strcat(domain,"&");
	strcat(domain,USERTOKEN);
	strcat(domain,"=");
	strcat(domain,_xm_epg.userToken);
	strcat(domain,"&");	
	strcat(domain,TYPE);	
	strcat(domain,"=");	
	strcat(domain,_xm_auth.type);
	return domain;
}

char *XM_AttachUserTimeshiftToken(char * domain, char start[7] , char end[7])
{
	strcat(domain,"?");
	strcat(domain,"starttime");
	strcat(domain,"=");
	//strcat(domain,"20140410221500");
	strcat(domain,start);
	strcat(domain,";");
	strcat(domain,"endtime");
	strcat(domain,"=");
	//strcat(domain,"20140410233600");
	strcat(domain,end);
