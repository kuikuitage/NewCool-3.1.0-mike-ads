...
static int  XM_DelCmdTask(void)
{
	...
	memset(xmDpBlock,0,sizeof(xmDpBlock));
	return 0;
}
...
static int  XM_UpdatePage( IPTV_UPPAGE_REQ_T  *p_req)
{
	...
	DEBUG(IPTVDP,INFO,"%s catid=%d,catid2=%d,page_index=%d, page_size=%d\n", __FUNCTION__,p_req->cat_id,p_req->cat_id2,
...
static int  XM_GetPlayUrlFormat(IPTV_VIDEO_URL_REQ_T *p_req)
{
	...
	int ret;     
	DP_CMD_T cmd;
	IPTV_VIDEO_URL_REQ_T * p_param = (IPTV_VIDEO_URL_REQ_T *)SY_MALLOC(sizeof(IPTV_VIDEO_URL_REQ_T));
	memset(p_param,0,sizeof(IPTV_VIDEO_URL_REQ_T));
...
{
	DEBUG(IPTVDP,INFO,"id_1 %d %d %d\n",id_1->res_id,id_1->cat_id,id_1->program_id);
	DEBUG(IPTVDP,INFO,"id_2 %d %d %d\n",id_2->res_id,id_2->cat_id,id_2->program_id);
