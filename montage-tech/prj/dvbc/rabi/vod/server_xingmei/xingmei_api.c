...
static void set_vdo_id(VDO_ID_t *id,int res_id,int cat_id,int pg_id,char * type,char *detail_url)
{
	if((strncasecmp(type,"Program",6)==0)||(type == NULL))
		id->type = IPTV_MOVIE;
	else if (strcasecmp(type,"tv_review")==0)
		id->type = IPTV_VARIETY_REVIEW;
	else
		id->type = IPTV_VARIETY;

	DEBUG(XM_GEN,INFO,"type %s   %d\n",type,id->type);	
	//id->type = type;
	id->res_id = res_id;
	id->cat_id = cat_id;
	id->program_id = pg_id;
	...
	memcpy(id->info_url,detail_url,strlen(detail_url)+1);
...
static al_iptv_name_res_list_t *xm_gen_name_res_list(XM_CATEGORY_LIST_T *p_param)
{
    ...
    {
        p_data = (al_iptv_name_res_list_t *)SY_CALLOC(1,sizeof(al_iptv_name_res_list_t));
        MT_ASSERT(p_data != NULL);

        p_data->total_res = (u16)p_param->total;
		p_data->resList = (al_iptv_name_res_item_t *)SY_CALLOC(1,p_data->total_res * sizeof(al_iptv_name_res_item_t));
...
static al_iptv_cat_res_list_t *xm_gen_cat_res_list(XM_CATEGORY_TYPE_ALL_T *p_param)
{
    ...
    {
        p_data = (al_iptv_cat_res_list_t *)SY_CALLOC(1,sizeof(al_iptv_cat_res_list_t));
        MT_ASSERT(p_data != NULL);

        p_data->total_res = (u16)p_param->category->total;
        p_data->resList = (al_iptv_cat_res_item_t *)SY_CALLOC(1,p_data->total_res * sizeof(al_iptv_cat_res_item_t));
        MT_ASSERT(p_data->resList != NULL);
...
static al_iptv_vdo_list_t *xm_gen_vdo_list(XM_CHANNEL_LIST_T *p_param)
{
	...
	p_data = (al_iptv_vdo_list_t *)SY_CALLOC(1,sizeof(al_iptv_vdo_list_t));
	MT_ASSERT(p_data != NULL);
	if(( NULL != p_param ) && (p_param->array != NULL) && (p_param->number > 0))
	{
		...
		p_data->vdoList = (al_iptv_vdo_item_t *)SY_CALLOC(1,p_data->vdo_cnt * sizeof(al_iptv_vdo_item_t));
		...
		for (i = 0; i < p_data->vdo_cnt; i++)
			...
			set_vdo_id(&p_data->vdoList[i].vdo_id,p_param->category_id,p_param->category_id2,palbum->id,palbum->type,palbum->detailUrl);
			{
				//VDO_ID_t *id_1 = &p_data->vdoList[i].vdo_id;
			...
			DEBUG(UI_PLAY_API,INFO,"[VDO IN]image[%s]\n",palbum->image);
			if (palbum->image[0] != '\0')
			{
...
static al_iptv_vdo_info_t *xm_gen_vdo_info(XM_CHANNEL_INFO_T *p_param)
{
	...
	{
		p_data = (al_iptv_vdo_info_t *)SY_CALLOC(1,sizeof(al_iptv_vdo_info_t));
		...
		p_data->vdo_id = p_param->id;

		{
			//VDO_ID_t *id_1 = &p_data->vdo_id;
			//DEBUG(XINGMEI,INFO,"id_1 %d %d %d\n",id_1->res_id,id_1->cat_id,id_1->program_id);
		}		
		...
		{
			p_data->format = SY_CALLOC(1,sizeof(al_iptv_source_format_t));
			MT_ASSERT(p_data->format != NULL);
			p_data->format->formatList = SY_CALLOC(1,sizeof(int)*p_param->format.total + 1);
...
{ 
	...
	p_data = (al_iptv_info_url_list_t *)SY_CALLOC(1,sizeof(al_iptv_info_url_list_t));
	MT_ASSERT(p_data != NULL);
	if (0 == strcasecmp(p_param->mov_detail.type,"Program")) //mov
	{

		p_data->count = 1;
		p_data->page_count = 1;
		p_data->page_total = 1;
		p_data->urlList = (al_iptv_info_url_item_t *)SY_CALLOC(1,p_data->count * sizeof(al_iptv_info_url_item_t));
		MT_ASSERT(p_data->urlList != NULL);

		p_data->urlList[0].id = p_param->id;
		src_len = strlen(p_param->mov_detail.name ) + 1;
		dest_len = src_len * sizeof(u16);
		inbuf =p_param->mov_detail.name ;
		outbuf = (char *)SY_CALLOC(1,dest_len);
	...
	else if (0 == strcasecmp(p_param->mov_detail.type,"tv_review"))
	{
		XM_scheduleList *sch = &p_param->mov_detail.schedule;
		char buf[128];
		if (p_param->episode_total > 0 && p_param->this_page_size > 0)
		{
			...
			DEBUG(UI_PLAY_API,INFO,"tv url total  %d\n",p_param->episode_total);
			DEBUG(UI_PLAY_API,INFO,"tv url page_count  %d\n",p_data->page_count);
			DEBUG(UI_PLAY_API,INFO,"tv url page_total  %d\n",p_data->page_total);
			
			p_data->count = (u32)p_param->episode_total;
			DEBUG(UI_PLAY_API,INFO,"tv_review :: episode_total = %d\n",(u32)p_param->episode_total);
			p_data->urlList = (al_iptv_info_url_item_t *)SY_CALLOC(1,p_data->count * sizeof(al_iptv_info_url_item_t));
			MT_ASSERT(p_data->urlList != NULL);

			p_data->epgList = (al_iptv_info_url_epg_lit_t *)SY_CALLOC(1,sch->total_days * sizeof(al_iptv_info_url_epg_lit_t));
			MT_ASSERT(p_data->epgList != NULL);

			p_data->epgDay = sch->total_days;
			for (i=sch->total_days;i>0;i--)
			{
				p_data->epgList[sch->total_days - i].count = sch->days[i-1].total;
				if(strlen(sch->days[i-1].date))
				{
					dest_len = strlen(sch->days[i-1].date) + 1;
					p_data->epgList[sch->total_days - i].epgTitle = (u8 *)SY_CALLOC(1,dest_len);
					strcpy(p_data->epgList[sch->total_days - i].epgTitle, sch->days[i-1].date);
					DEBUG(UI_PLAY_API,INFO,"%s [%d]\n",p_data->epgList[sch->total_days - i].epgTitle,sch->days[i-1].total);
				}
				
				for (ii=0;ii<sch->days[i-1].total;ii++)
				{
					if (count >= p_data->count)
						break;

					p_data->urlList[count].id = p_param->id;

					memset(buf,0,sizeof (buf));
					strcat(buf,sch->days[i-1].schedules[ii].start_time);
					strcat(buf,"--");					
					strcat(buf,sch->days[i-1].schedules[ii].end_time);
					strcat(buf,"  ");
					strcat(buf,sch->days[i-1].schedules[ii].name);

					src_len = strlen(buf) + 1;
					dest_len = src_len * sizeof(u16);
					inbuf = buf ;
					outbuf = (char *)SY_CALLOC(1,dest_len);
					MT_ASSERT(outbuf != NULL);
					p_data->urlList[count].urltitle = (u16 *)outbuf;
					iconv(g_cd_utf8_to_utf16le, (char**) &inbuf, &src_len, (char**) &outbuf, &dest_len);
			...
			{
				DEBUG(UI_PLAY_API,INFO,"urltitle[%s]\n",p_data->urlList[i].urltitle);
			}
			return p_data;
		}
	}
	else
	{
		if (p_param->episode_total > 0 && p_param->this_page_size > 0)
		{
			...
			for (i = 0; i < p_data->count; i++)
			{
				pepisode = p_param->mov_detail.seriesList.data+ i;
				src_len = sizeof(pepisode->name ); 
				DEBUG(UI_PLAY_API,INFO,"url name[%s]len[%d]\n",pepisode->name,src_len );
				p_data->urlList[i].id = p_param->id;
...
static void xm_free_info_url(u32 param)
{
    ...
    if (p_data)
    {
    	if(p_data->epgList)
    	{
			...
			{
				if(p_data->epgList[i].epgTitle)
					SY_FREE(p_data->epgList[i].epgTitle);
			}
			SY_FREE(p_data->epgList);
		}
...
static al_iptv_source_format_t *xm_gen_source_format(XM_VIDEO_FORMAT_T * format)
{
	...
	int i;
	DEBUG(UI_PLAY_API,INFO,"format total %d\n",format->total);

	p_data = (al_iptv_source_format_t *)SY_CALLOC(1,sizeof(al_iptv_source_format_t));
	MT_ASSERT(p_data != NULL);

	p_data->total_format = 0;
	p_data->formatList = (int *)SY_CALLOC(1,IPTV_MAX_FORMAT_COUNT * sizeof(*p_data->formatList));
...
static al_iptv_play_url_list_t *xm_gen_play_url(XM_CHANNEL_PLAY_URL_T *p_param)
{
	...
	{
		p_data = (al_iptv_play_url_list_t *)SY_CALLOC(1,sizeof(al_iptv_play_url_list_t));
		MT_ASSERT(p_data != NULL);

		p_data->total_url = (u16)p_param->number;
		p_data->pp_urlList = (al_iptv_play_url_info_t*)SY_CALLOC(1,p_data->total_url * sizeof(p_data->pp_urlList[0]));
		...
		for (i = 0; i < p_data->total_url; i++)
		{
			...
			if (strlen(p_item->playUrl) != 0)
			{
				...
				DEBUG(XM_GEN,INFO,"URLplayUrl = %s\n",p_data->pp_urlList->playUrl);
			}
		}
...
static al_iptv_recomand_info_t *xm_gen_remend_info(XM_RECMD_INFO_T *p_param)
{
	...
	p_data = (al_iptv_recomand_info_t *)SY_CALLOC(1,sizeof(al_iptv_recomand_info_t));
	MT_ASSERT(p_data != NULL);
	...
	p_data->recmd  = (iptv_recmd_channel_t *)SY_CALLOC(1,p_data->number * sizeof(iptv_recmd_channel_t));
	MT_ASSERT(p_data->recmd != NULL);

	for (i = 0; i < p_data->number; i++)
	{
		p_item = p_param->recmd+ i;
		memcpy(p_data->recmd[i].id.info_url,p_param->recmd[i].detailUrl,strlen(p_param->recmd[i].detailUrl)+1);
		...
		*/
		if((strncasecmp(p_item->type,"Program",6)==0)||(p_item->type == NULL))
		p_data->recmd[i].id.type = IPTV_MOVIE;
		else
		p_data->recmd[i].id.type = IPTV_VARIETY;
...
static al_iptv_search_word_t *xm_gen_search_word(XM_SEARCH_WORD_T *p_param)
{
    ...
    {
        p_data = (al_iptv_search_word_t *)SY_CALLOC(1,sizeof(al_iptv_search_word_t));
        MT_ASSERT(p_data != NULL);

        p_data->total_word = (u16)p_param->search->wordSize;
        p_data->word_list = (al_iptv_word_name_t *)SY_CALLOC(1,p_data->total_word * sizeof(al_iptv_word_name_t));
...
void xm_dp_init(void)
{
	...
	config.mac = SY_CALLOC(1,MAC_CHIPID_LEN);
	config.id = SY_CALLOC(1,MAC_CHIPID_LEN);
