...
#define 	 XM_PLAY_URL_NUM		    200
#define 	 XM_RECOMMAND_MAX_NUM		    50
#define 	 XM_NAME_MAX_LEN1			96


#define XM_SCHEDULE_DAYS			7
#define XM_SCHEDULE_MAX_PER_DAY	150
...
typedef struct st_XM_homepage_C
{
	...
	char review_entry_url[XM_LEN_URL];
...
typedef struct st_XM_mov_C
{
	...
	/*for live review*/
	unsigned int timeshiftLength;
	unsigned int storageLength;
	char currentSchedule[XM_LEN_URL];	
	char scheduleListUrl[XM_LEN_URL];
...
{
	int id;
	char name[XM_NAME_MAX_LEN1]; //多少集第几集
...
/*  review schedule*/
typedef struct st_XM_schedule_s
{
	char name[XM_NAME_MAX_LEN1];
	char start_time[16];
	char end_time[16];
} XM_schedule;

typedef struct st_XM_schedule_day_s
{
	int total;
	char date[16];
	XM_schedule schedules[XM_SCHEDULE_MAX_PER_DAY];
}XM_scheduleDay;

typedef struct st_XM_schedule_list_C
{
	int 	total_days;									//回看天数
	XM_scheduleDay days[XM_SCHEDULE_DAYS];		  	//回看列表
} XM_scheduleList;
...
typedef struct st_XM_detail_C
{
	...
	XM_scheduleList	schedule;//回看节目表
} XM_detail;
...
{							
	int	  num; 						
	XM_recommand data[XM_RECOMMAND_MAX_NUM];	
} XM_RecmdList;
...
typedef struct st_XM_play_urlList_C
{
	...
	int num;
	XM_urlobj urlobj[XM_PLAY_URL_NUM];//播放地址集合
...
char *XM_AttachUserTimeshiftToken(char * domain, char start[6] , char end[6]);
