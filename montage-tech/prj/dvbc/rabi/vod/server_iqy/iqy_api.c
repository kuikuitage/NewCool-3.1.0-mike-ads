...
static al_iptv_info_url_list_t *iqy_gen_info_url(IQY_CHANNEL_INFO_T *p_param)
{
	...
	if (p_param->album->type == IQY_TYPE_MOVE) //move
	{
		...
		memcpy((char *) p_data->urlList[0].id.tvQid, (char *)p_param->album->tvQid,sizeof(p_data->urlList[0].id.tvQid));
		memcpy((char *) p_data->urlList[0].id.vid, (char *)p_param->album->vid,sizeof(p_data->urlList[0].id.vid));
		DEBUG(UI_PLAY_API,INFO,"tvQid[%s],vid[%s]\n",p_data->urlList[0].id.tvQid,p_data->urlList[0].id.vid);
	...
	for (i = 0; i < p_data->count; i++)
	{
		...
		memcpy((char *) p_data->urlList[i].id.tvQid, (char *)pepisode->tvQid,sizeof(p_data->urlList[0].id.tvQid));
		memcpy((char *) p_data->urlList[i].id.vid, (char *)pepisode->vid,sizeof(p_data->urlList[0].id.vid));
