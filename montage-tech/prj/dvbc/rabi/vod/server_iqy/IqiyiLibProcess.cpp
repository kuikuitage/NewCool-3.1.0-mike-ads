...
static void *iqy_get_url(void * hdl, IPTV_VIDEO_URL_REQ_T * p_req)
{
	...
	{
		memcpy(play_url->playurls[i].tvQid,p_req->id.tvQid,sizeof(play_url->playurls[0].tvQid));
		memcpy(play_url->playurls[i].vid,p_req->id.vid,sizeof(play_url->playurls[0].vid));
	}
	...
	for (i=0;i<vlist.size && i<IPTV_MAX_FORMAT_COUNT;i++)
	{
		switch(vlist.vdlist[i])
		{
			...
			case TYPE_VD_FAST:        //96: 极速
				...
				format->format[count++] = IPTV_API_FORMAT_TOPSPEED;
...
static void* iqy_get_format(void * hdl, IPTV_VIDEO_URL_REQ_T * p_req)
{
	...
	DEBUG(IQY,INFO,"tvQid[%s],vid[%s]\n",p_req->id.tvQid,p_req->id.vid);
	priv->cur_episode_num  =  p_req->episode_num;
	_get_format(hdl, format, p_req->id.tvQid, p_req->id.vid);
	return format;
}
