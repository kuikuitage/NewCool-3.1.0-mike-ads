...
void ui_iptv_get_video_list(u32 res_id,u32 cat_id ,u16 *cat_name, u8 *key, u32 page_num)
{
	char *inbuf, *outbuf;
	...
	if (pvod != NULL)
	{
		...
		req.cat_id2 = cat_id;//新媒根据二级的CAT_NAME获取到二级的CAT_ID
		if (cat_name)
		{
			inbuf = (char *)cat_name;
...
void ui_iptv_get_video_info(VDO_ID_t *vdo_id, u8 cat_id)//This cat_id is type of the Program or series for xinmei
{
#if 1//SY DELETE
	const VodDpInterface_t * pvod = ui_iptv_get_instance();
...
void ui_iptv_get_play_format(VDO_ID_t *vdo_id,int episode_num)
{
...
void ui_iptv_get_play_url(IPTV_API_FORMAT_T format,VDO_ID_t *vdo_id,int episode_num)
{
	IPTV_VIDEO_URL_REQ_T req;
	const VodDpInterface_t * pvod = ui_iptv_get_instance();
