  ...
  switch(p_priv->nc_tuner[tuner_id].status)
  {
    ...
    case NC_MONITOR:
      if(p_priv->nc_tuner[tuner_id].monitor_enable)
      {
        ...
        {
          if(p_priv->nc_tuner[tuner_id].cur_lock_mode == SYS_DTMB)
          {
            nim_lock_state_t  lock_state = NIM_LOCK_STATE_UNDEF;
            
            dev_io_ctrl(p_priv->nc_tuner[tuner_id].p_nim_dev, NIM_IOCTRL_CHECK_LOCK, 
                (u32)&lock_state);
            
//            mtos_printk("lock state = %d\n", lock_state);
            switch(lock_state)
            {
              case NIM_LOCK_STATE_UNLOCKED:
                p_priv->nc_tuner[tuner_id].duration_ticks = 0;
                break;
              case NIM_LOCK_STATE_LOCKED:
                is_lock = TRUE;
                break;
              case NIM_LOCK_STATE_WAITING:
                break;
              default:
                break;
            }
