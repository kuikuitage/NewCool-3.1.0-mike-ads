                                           ...
                                           nim_channel_perf_t *p_perf)
{
  ...
  if(p_priv->mt_dd_handle->demod_cur_mode == MtFeType_CTTB)
  {
    ...
    _mt_fe_dmd_get_ber_dd3k_c(p_priv->mt_dd_handle, (U32 *)&p_perf->ber, (U32 *)&p_perf->dvbc_total);
  ...
  p_channel_set_info)
{
  ...
  p_channel_set_info->lock_time = 6000; //2s
...
static RET_CODE nim_m88dd3k_ioctrl(lld_nim_t *p_lld, u32 cmd, u32 param)
{
  ...
  switch(cmd)
  {
    ...
    case NIM_IOCTRL_CHANNEL_CHECK_LOCK:
      mt_fe_dmd_get_lock_state_dd3k(p_priv->mt_dd_handle, &status);
      ((nim_channel_info_t *)param)->lock = status;
        //(status == MtFeLockState_Locked) ? 1 : 0;
      break;

    case NIM_IOCTRL_CHECK_LOCK:
      mt_fe_dmd_get_lock_state_dd3k(p_priv->mt_dd_handle, &status);
      *((u8 *)param) = status;//(status == MtFeLockState_Locked) ? 1 : 0;
      break;
    ...
    case NIM_IOCTRL_GET_ERR_PACKAGES:
 	...
 	case NIM_IOCTRL_GET_CHANNEL_INFO:
      ...
      break;		
	case NIM_IOCTRL_DUMP_REGS: 
	  reg_type = (*(u32*)param >> 16) & 0xffff;
	  if (reg_type == NIM_REG_DTMB)
	  {
    	dmd_reg_addr = (*(u32*)param) & 0xffff;
		dd3k_handle->dmd_get_reg(dd3k_handle, dmd_reg_addr, &reg_data);
		//DS3K_PRINT("dmd_reg_addr %x reg_data %x\n", dmd_reg_addr, reg_data);
 	  }
      else if(reg_type == NIM_REG_TUNER0)
      {
    	tun_reg_addr = (*(u32*)param) & 0xffff;
		dd3k_handle->tn_read(dd3k_handle, &tun_reg_addr, 1, &reg_data, 1);
	    //DS3K_PRINT("tc3800 tun_reg_addr %x reg_data %x\n",tun_reg_addr, reg_data);
	  }
	  *(u32 *)param = reg_data;
	  break;
 	case NIM_IOCTRL_PRINT_CHANNEL_INFO:
  	  for(i = 0; i < 10; i++)
  	  {
  	    mt_fe_dmd_get_lock_state_dd3k(p_priv->mt_dd_handle, &status);
  		mtos_task_delay_ms(10);
  		if(status == NIM_LOCK_STATE_LOCKED)
  		{
  		  mt_fe_dmd_coeff_iqata_dd3k(dd3k_handle);
  		  break;
  		}
  	  }
	  break;
