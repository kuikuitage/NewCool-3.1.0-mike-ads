...
*	Modify			2.80.20		2016.02.23		WangBingju
*	Modify			2.80.20		2016.03.09		WangBingju
********************************************************************************************************/
...
MT_FE_RET _mt_fe_dmd_reg_init_dd3k_t(MT_FE_DD_Device_Handle  handle)
{
	...
	else if(handle->demod_id == MtFeDmdId_DD630X)
	{
		...
		handle->dmd_set_reg(handle, 0x015f, 0x8d);
		handle->dmd_set_reg(handle, 0x0179, 0x0a);
		////adc
		data1 = 0;
...
MT_FE_RET mt_fe_dmd_coeff_iqata_dd3k(MT_FE_DD_Device_Handle  handle)
{
    U8 data = 0;
    S32 num = 0, idata = 0, qdata = 0, i = 0;
    ...
    if(handle->demod_id == MtFeDmdId_DD630X)
    {
        ...
        {
            handle->dmd_get_reg(handle, 0x01cd, &data);
            if ((data & 0x01) == 0x01)
            {
                handle->dmd_get_reg(handle, 0x00bc, &data);
                if((data&0x03) == 0x00)
                {
                	num = 420;
                }
                ...
                handle->dmd_get_reg(handle, 0x0240, &data);
                data = data & 0x7f;
                handle->dmd_set_reg(handle, 0x0240, data);
                handle->dmd_set_reg(handle, 0x0260, 0x6e);
                for(i=0; i<num; i++)
                {
	                data = 0;
	                handle->dmd_get_reg(handle, 0x0261, &data);
	                idata = (S32) data;
	                data = 0;
	                handle->dmd_get_reg(handle, 0x0261, &data);
	                idata = (((S32) data&0x0f)<<8)+idata;
	                if(idata>0x7ff)
	                	idata = idata -0x1000;
	                qdata = (S32)(data& 0xf0) >>4;
	                data = 0;
	                handle->dmd_get_reg(handle, 0x0261, &data);
	                qdata = (((S32)data) << 4) + qdata;
	                if(qdata>0x7ff)
	                	qdata = qdata -0x1000;
	                mtos_printk("%d  %d\n", idata, qdata);
                }
                ...
                data = data | 0x80;
                handle->dmd_set_reg(handle, 0x0240, data);
                handle->dmd_set_reg(handle, 0x0260, 0x2e);
			}
            else
            {
            	return MtFeErr_Fail;
